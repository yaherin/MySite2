    ...
    for (row = 0; row < 2; row++) {
        ...
        for (col = 0; col < 7; col++) {
            if ((row === 0 && col === 6) || (row === 1 && col === 0)) {
                if (row === 0 && col === 6) {
                    textspecial = textspecial + "<div class='storep1' id='" + num + "' ;' onclick='StartGame(this);'> <div class='label'>" + mancala[row][col] + "</div> <img src = '" + ImgArray[mancala[row][col]] + "'  width = '110%'; height = '45%' /> </div > ";
                }
                else if (row === 1 && col === 0) {
                    textspecial = textspecial + "<div class='storep2' id='" + num + "' ;' onclick='StartGame(this);'> <div class='label'>" + mancala[row][col] + " </div> <img src = '" + ImgArray[mancala[row][col]] + "'  width = '110%'; height = '45%' /> </div > ";
                }
                num++;
            }
            else {
                text = text + "<div class='pit' id='" + num + "' ;' onclick='StartGame(this);'> <div class='label'>" + mancala[row][col] + "</div> <img src = '" + ImgArray[mancala[row][col]] + "'  width = '120%'; height = '100%'; align='top'/>  </div > ";
                num++;
            }
...
function IsEndGame() {
    ...
    if (nonep1 || nonep2) {
        ...
        if (mancala[0][6] > mancala[1][0]) {
            alert("Player 1 won!");
            document.getElementById("turns").innerHTML = "Hurray! " + p1name + " won!";
            AudioWin.play();
        }
        else if (mancala[0][6] == mancala[1][0]) {
            alert("Tie!");
            document.getElementById("turns").innerHTML = "Oh! We have a tie!";
            AudioTie.play();
        }
        else {
            alert("Player 2 won!");
            document.getElementById("turns").innerHTML = "Hurray! " + p2name + " won!";
            AudioWin.play();
        }
